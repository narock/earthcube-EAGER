package earthcube.eager.write.oceanlink;

import earthcube.eager.util.Constants;

public class PersonName {
	
	public String toRdfXml ( String personNameUri, String fullName, String firstName, String lastName ) {
	
		String output = 
			
			"<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + Constants.newLine +
			"<rdf:RDF xmlns=\"http://schema.oceanlink.org/person-name#\"" + Constants.newLine +
			"   xmlns:owl=\"http://www.w3.org/2002/07/owl#\"" + Constants.newLine +
			"   xmlns:rdf=\"http://www.w3.org/1999/02/22-rdf-syntax-ns#\"" + Constants.newLine +
			"   xmlns:rdfs=\"http://www.w3.org/2000/01/rdf-schema#\"" + Constants.newLine +
			"   xmlns:time=\"http://www.w3.org/2006/time#\"" + Constants.newLine +
			"   xmlns:xsd=\"http://www.w3.org/2001/XMLSchema#\"" + Constants.newLine +
			"   xml:base=\"http://schema.oceanlink.org/person-name\">" + Constants.newLine +
			"   <rdf:Description rdf:about=\"" + personNameUri +"\">" + Constants.newLine +
			"      <rdf:type rdf:resource=\"PersonName\"/>" + Constants.newLine +
			"      <rdfs:subClassOf rdf:resource=\"PersonalInfoValue\"/>" + Constants.newLine;
		
		    if ( fullName != null ) {
			  output += "      <fullNameAsString rdf:datatype=\"xsd:string\">" + fullName + "</fullNameAsString>" + Constants.newLine;
		    }
		    
		    if ( firstName != null ) {
			  output += "      <firstOrGivenName rdf:datatype=\"xsd:string\">" + firstName + "</firstOrGivenName>" + Constants.newLine;
		    }
		    
		    if ( lastName != null ) {
			  output += "      <familyOrSurname rdf:datatype=\"xsd:string\">" + lastName + "</familyOrSurname>" + Constants.newLine;
		    }
		    
		    output += "   </rdf:Description>" + Constants.newLine + "</rdf:RDF>";
		
		return output;
			  
	}
	
}